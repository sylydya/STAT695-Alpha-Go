  ...
  def build_model_from_tfrecords_features(features,mode,print_model,trainlog,model_config,pos_len,num_batches_per_epoch,lr_scale=None):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      with tf.control_dependencies(update_ops):
        ...
        adjusted_gradients = []
        #---------- adaptively update cdf in energy ----------------------#
        
        '''
        target_vars_opt_loss = np.random.uniform(3., 4.)
        J = min(max(int((target_vars_opt_loss - 2.) / 0.05 + 1.), 1.), 49.)
        grad_mul = 1 +  (1 - Gcum[J-1] / Gcum[J]) / 0.05
        # update CDF in energy
        Gcum[J:] = 0.9 * Gcum[J:] + 0.1 * Gcum[J]
        Gcum[:J] = 0.9 * Gcum[:J]
        Gcum = Gcum / Gcum[-1]
        '''
        for (grad,x) in gradients:
          #adjusted_grad = grad * min(max(grad_mul, 3.), 1.) / 2.
          adjusted_grad = grad * target_vars.opt_loss ** 0.5 / 2.
