...
class Model:
  ...
  def set_prune_mask(self, threshold):
    prune_mask = []
    prune_op = []
    for variable in tf.trainable_variables():
      my_temp = tf.cast(tf.abs(variable) > threshold, tf.float32)
      prune_mask.append(my_temp)
      prune_op.append(tf.assign(variable, variable * my_temp))
...
class ModelUtils:
  ...
  @staticmethod
  def build_model_from_tfrecords_features(features,mode,print_model,trainlog,model_config,pos_len,num_batches_per_epoch,lr_scale=None, pruned_yet = False, do_prune = False, First_prune = False):
...
#----------mixture normal prior parameter----------------------#
        ...
        if (do_prune and not pruned_yet):
          prune_op = model.set_prune_mask(threshold)


        for num, (grad,x) in enumerate(gradients):
          adjusted_grad = grad
          ...
          if do_prune:
            adjusted_grad = adjusted_grad*model.prune_mask[num]
      ...
      if do_prune and not pruned_yet:
        return (model,target_vars,metrics,global_step,global_step_float,per_sample_learning_rate,train_step, prune_op)
