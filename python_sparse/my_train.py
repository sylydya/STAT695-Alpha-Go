...
args = {}
traindir = args["traindir"] = '/scratch/gilbreth/sun748/result/train/train_sparse_prune/'
datadir = args["datadir"] = '/scratch/gilbreth/sun748/result/shuffleddata/current/'
exportdir = args["exportdir"] = '/scratch/gilbreth/sun748/result/tfsavedmodels_toexport'
exportprefix = args["exportprefix"] = 'train_sparse_prune'
pos_len = args["pos_len"] = 9
batch_size = args["batch_size"] = 256
samples_per_epoch = args["samples_per_epoch"] = 1000000
gpu_memory_frac = args["gpu_memory_frac"] = 0.6
model_kind = args["model_kind"] = 'b20c256'
lr_scale = None
sub_epochs = args["sub_epochs"] = 4
swa_sub_epoch_scale = args["swa_sub_epoch_scale"] = 4
verbose = args["verbose"] = None
no_export = args["no_export"] = None
logfilemode = "a"
...
def model_fn(features, labels, mode, params):
  ...
  pruned_yet_temp = pruned_yet
  do_prune_temp = do_prune

  print_model = not printed_model_yet

  built = ModelUtils.build_model_from_tfrecords_features(features, mode, print_model, trainlog, model_config, pos_len,
                                                         num_batches_per_epoch, lr_scale, pruned_yet_temp,
                                                         do_prune_temp, First_prune)
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    return tf.estimator.EstimatorSpec(
      ...
      eval_metric_ops={
        ...
        "sbpdfloss": tf.metrics.mean(target_vars.scorebelief_pdf_loss_unreduced,
                                     weights=target_vars.target_weight_used),
        "sbcdfloss": tf.metrics.mean(target_vars.scorebelief_cdf_loss_unreduced,
                                     weights=target_vars.target_weight_used),
        "bbpdfloss": tf.metrics.mean(target_vars.bonusbelief_pdf_loss_unreduced,
                                     weights=target_vars.target_weight_used),
        "bbcdfloss": tf.metrics.mean(target_vars.bonusbelief_cdf_loss_unreduced,
                                     weights=target_vars.target_weight_used),
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    if do_prune and not pruned_yet:
      (model, target_vars, metrics, global_step, global_step_float, per_sample_learning_rate, train_step,
       prune_op) = built
      if First_prune:
        First_prune = not First_prune
    ...
    (p0loss, p0loss_op) = moving_mean("p0loss", target_vars.policy_loss_unreduced,
                                      weights=target_vars.target_weight_used)
    (p1loss, p1loss_op) = moving_mean("p1loss", target_vars.policy1_loss_unreduced,
                                      weights=target_vars.target_weight_used)
    (vloss, vloss_op) = moving_mean("vloss", target_vars.value_loss_unreduced, weights=target_vars.target_weight_used)
    (smloss, smloss_op) = moving_mean("smloss", target_vars.scoremean_loss_unreduced,
                                      weights=target_vars.target_weight_used)
    (sbpdfloss, sbpdfloss_op) = moving_mean("sbpdfloss", target_vars.scorebelief_pdf_loss_unreduced,
                                            weights=target_vars.target_weight_used)
    (sbcdfloss, sbcdfloss_op) = moving_mean("sbcdfloss", target_vars.scorebelief_cdf_loss_unreduced,
                                            weights=target_vars.target_weight_used)
    (bbpdfloss, bbpdfloss_op) = moving_mean("bbpdfloss", target_vars.bonusbelief_pdf_loss_unreduced,
                                            weights=target_vars.target_weight_used)
    (bbcdfloss, bbcdfloss_op) = moving_mean("bbcdfloss", target_vars.bonusbelief_cdf_loss_unreduced,
                                            weights=target_vars.target_weight_used)
    (uvloss, uvloss_op) = moving_mean("uvloss", target_vars.utilityvar_loss_unreduced,
                                      weights=target_vars.target_weight_used)
    (oloss, oloss_op) = moving_mean("oloss", target_vars.ownership_loss_unreduced,
                                    weights=target_vars.target_weight_used)
    (rwlloss, rwlloss_op) = moving_mean("rwlloss", target_vars.winloss_reg_loss_unreduced,
                                        weights=target_vars.target_weight_used)
    (rsmloss, rsmloss_op) = moving_mean("rsmloss", target_vars.scoremean_reg_loss_unreduced,
                                        weights=target_vars.target_weight_used)
    (rsdloss, rsdloss_op) = moving_mean("rsdloss", target_vars.scorestdev_reg_loss_unreduced,
                                        weights=target_vars.target_weight_used)
    (roloss, roloss_op) = moving_mean("roloss", target_vars.ownership_reg_loss_unreduced,
                                      weights=target_vars.target_weight_used)
    (rloss, rloss_op) = moving_mean("rloss", target_vars.reg_loss_per_weight, weights=target_vars.weight_sum)
    (rscloss, rscloss_op) = moving_mean("rscloss", target_vars.scale_reg_loss_unreduced,
                                        weights=target_vars.target_weight_used)
    ...
    (ptentr, ptentr_op) = moving_mean("ptentr", metrics.policy_target_entropy_unreduced,
                                      weights=target_vars.target_weight_used)
    ...
    if do_prune and not pruned_yet:
      ...
      return tf.estimator.EstimatorSpec(
        mode,
        loss=(target_vars.opt_loss / tf.constant(batch_size, dtype=tf.float32)),
        train_op=tf.group(prune_op, train_step, p0loss_op, p1loss_op, vloss_op, smloss_op, sbpdfloss_op, sbcdfloss_op,
                          bbpdfloss_op, bbcdfloss_op,
                          uvloss_op, oloss_op, rwlloss_op, rsmloss_op, rsdloss_op, roloss_op, rloss_op, rscloss_op,
                          pacc1_op, ventr_op, ptentr_op, wmean_op),
    ...
    return tf.estimator.EstimatorSpec(
      mode,
      loss=(target_vars.opt_loss / tf.constant(batch_size, dtype=tf.float32)),
      train_op=tf.group(train_step, p0loss_op, p1loss_op, vloss_op, smloss_op, sbpdfloss_op, sbcdfloss_op, bbpdfloss_op,
                        bbcdfloss_op,
                        uvloss_op, oloss_op, rwlloss_op, rsmloss_op, rsdloss_op, roloss_op, rloss_op, rscloss_op,
                        pacc1_op, ventr_op, ptentr_op, wmean_op),
      training_hooks=[logging_hook]
...
print(estimator.get_variable_value('rconv1/w1'))
#
# lambda_n = 0.0000001
# prior_sigma_0 = 0.000002
# prior_sigma_1 = 0.2
#
# N_temp = 1 / 0.00003 / (2 * prior_sigma_1)
#
...
#
# element_list = []
# large_element_list = np.zeros(tf.trainable_variables().__len__())
# total_element_list = np.zeros(tf.trainable_variables().__len__())
# non_zero_list = np.zeros(tf.trainable_variables().__len__())
# for temp_index, temp_v in enumerate(tf.trainable_variables()):
#   temp_name = temp_v.name.split(":")[0][10:]
#   temp_value = estimator.get_variable_value(temp_name)
#   element_list.append(temp_value)
#   # print(temp_name)
#   size = 1
#   for dim in temp_value.shape:
#     size = size*dim
#   total_element_list[temp_index] = size
#   large_element_list[temp_index] = np.sum((np.abs(temp_value) > threshold))
#   non_zero_list[temp_index] = np.sum(temp_value != 0)
# import pickle
# filename = "sparse_result.txt"
# f = open(filename, 'wb')
# pickle.dump([element_list, large_element_list, total_element_list, non_zero_list], f)
# f.close()
# non_zero_list.sum()/total_element_list.sum()
# large_element_list.sum()/total_element_list.sum()
...
while True:
  ...
  for i in range(sub_epochs):
    ...
    # print(estimator.get_variable_value(estimator.get_variable_names()[101]))
    # print(estimator.get_variable_value(estimator.get_variable_names()[89]))
    print(estimator.get_variable_value('rconv1/w1'))
